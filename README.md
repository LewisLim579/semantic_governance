...
**회사 데이터를 AI·분석이 올바르게 이해하도록, 업무 용어·관계·규칙을 사람이 읽고 고칠 수 있는 형태(YAML)로 모아 Git에서 관리하는 "업무 의미 사전(시맨틱 레이어)"입니다.**

AI Agent(챗봇·분석 비서)가 "매출할인이 무엇인지", "주문과 고객이 어떻게 연결되는지", "쿠폰은 중복 적용이 안 된다"는 것을 알아야 정확히 답할 수 있습니다. 이 저장소는 그런 **업무 지식**을 한곳에 모아 두고, 버전관리·검토·자동검증을 거쳐 AI가 사용하는 시스템(그래프DB·벡터DB)에 자동 반영되도록 합니다.

데이터(숫자·표) 자체는 **Databricks**가, AI 실행은 **별도 런타임**이 담당하며, **이 저장소는 그 사이의 "의미"만** 책임집니다.

---

## 여기서 무엇을 관리하나요?

업무 지식을 **5가지 구성요소**로 나누고, 그것을 **도메인(업무 분야)별 폴더**에 YAML로 관리합니다.

### (1) 5가지 구성요소 — 무엇을 담는가

| 파일(구성요소) | 여기에 관리하는 내용 | 예시 |
|---|---|---|
| `concept.yaml` · **개념** | 업무 용어·대상과 그 속성·단위·동의어·계산식 | 주문(Order), 판매량(kg), TMC(원/kg) |
| `relation.yaml` · **관계** | 개념끼리 어떻게 연결되는지 | 주문 → 고객(주문자), 캠페인 → 상품 |
| `rule.yaml` · **규칙** | 지켜야 할 업무 규칙·제약 | 쿠폰 중복 적용 불가, 예산 한도 |
| `mapping.yaml` · **데이터 연결** | 개념이 실제 데이터(Databricks 표)의 어느 컬럼과 연결되는지 | Order ↔ `gold.trading.orders` |
| `prompt.yaml` · **AI 지침** | 도메인 AI 비서의 역할·답변 규칙 | "답변에 출처를 함께 제시" |

### (2) 도메인 — 어떤 업무 분야를 담는가

| 폴더 | 업무 분야 | 관리 범위(예) |
|---|---|---|
| `common/` | 공통 | 여러 도메인이 함께 쓰는 개념 — 거래·고객·상품·충전소·판매량·사내이자율 |
| `domains/trading/` | 거래 | 주문·쿠폰, 주문-고객 관계, 쿠폰/할인 규칙 |
| `domains/finance/` | 재무 | 결제·계좌, 정산·환불 규칙 |
| `domains/marketing/` | 마케팅 | 캠페인·세그먼트, 예산·수신동의 규칙 |
| `domains/station_sales/` | 충전소 영업분석 | TMC 지표군(판매량·할인·수송비·지원비·TMC·기여이익)과 계산식·부호/제외·연단위 규칙 |
| `domains/membership/` | 멤버십 | 회원·가입·충전실적, 멤버십비중, 코드매핑(HMS·OCB) 규칙 |

### (3) 한 줄 정리

| 질문 | 답 |
|------|----|
| 무엇인가? | 데이터에 업무적 의미를 부여하는 **시맨틱 레이어의 원천(SoT) 저장소** |
| 왜 필요한가? | AI·분석이 데이터를 **일관되고 정확하게** 해석하도록 용어·관계·규칙을 한곳에서 관리 |
| 누가 쓰나? | **현업**(용어·규칙을 UI로 수정) + **IT·데이터팀**(데이터 연결·검증·적재) |
| 어떻게 관리하나? | Git에서 **코드처럼**(PR·리뷰·자동검증·버전) 관리 → 통과 시 AWS에 적재 |

> 비유하자면, 데이터(숫자·표)가 "재료"라면 이 저장소는 그 재료의 **이름표·레시피·주의사항**입니다.
> 재료(Databricks)와 요리사(AI)는 그대로 두고, 사전·레시피만 여기서 고치면 전체가 같은 기준으로 움직입니다.
